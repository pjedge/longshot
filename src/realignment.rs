...
use util::{AlignmentParameters, TransitionProbs, LnTransitionProbs, LnAlignmentParameters};
...
pub fn sum_all_alignments(v: &Vec<char>,
    ...
    middle_prev[0] = 1.0;
    upper_prev[1] = params.eq.deletion_from_match;

    for j in 2..(w.len() + 1) {
        upper_prev[j] = upper_prev[j - 1] * params.eq.deletion_from_deletion;
        middle_prev[j] = 0.0;
        //lower_prev[j] = 0.0;
    }
    ...
    for i in 1..(v.len() + 1) {
        ...
        if band_start == 1 {
            ...
            middle_curr[0] = 0.0;
            if i == 1 {
                lower_curr[0] = params.eq.insertion_from_match
            } else {
                lower_curr[0] = lower_prev[0] * params.eq.insertion_from_insertion;
            }
        }

        for j in band_start..(band_end + 1) {

            let probs: TransitionProbs = match v.get(i - 1) {
                Some(v_i) => {
                    match w.get(j - 1) {
                        Some(w_j) => {
                            if v_i == w_j {
            ...
            let lower_continue = lower_prev[j] * probs.insertion_from_insertion;
            let lower_from_middle = middle_prev[j] * probs.insertion_from_match;
            lower_curr[j] = lower_continue + lower_from_middle;

            let upper_continue = upper_curr[j - 1] * probs.deletion_from_deletion;
            let upper_from_middle = middle_curr[j - 1] * probs.deletion_from_match;
            upper_curr[j] = upper_continue + upper_from_middle;

            let middle_from_lower = lower_prev[j - 1] * probs.match_from_insertion;
            let middle_continue = middle_prev[j - 1] * probs.match_from_match;
            let middle_from_upper = upper_prev[j - 1] * probs.match_from_deletion;
...
pub fn sum_all_alignments_numerically_stable(v: &Vec<char>,
    ...
    for i in 1..(v.len() + 1) {
        ...
        for j in band_start..(band_end + 1) {
            //for j in 1..(w.len() + 1) {
            let probs: LnTransitionProbs = match v.get(i - 1) {
                Some(v_i) => {
                    match w.get(j - 1) {
                        Some(w_j) => {
                            if v_i == w_j {
